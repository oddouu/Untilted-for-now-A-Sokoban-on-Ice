class Character {
  ...
  placeAt(x, y) {
    ...
    this.position = [
      game.map.tileW * x + (game.map.tileW - this.dimensions[0]) / 2,
      game.map.tileH * y + (game.map.tileH - this.dimensions[1]) / 2
    ];
  }
  ...
  processMovement(t) {
    ...
    if (t - this.timeMoved >= this.delayMove) {
      ...
      let tileFloor =
        game.map.tileTypes[
          game.map.gameMap[game.map.toIndex(this.tileFrom[0], this.tileFrom[1])]
        ].floor;
      ...
      this.position[0] =
        this.tileFrom[0] * game.map.tileW +
        (game.map.tileW - this.dimensions[0]) / 2;
      this.position[1] =
        this.tileFrom[1] * game.map.tileH +
        (game.map.tileH - this.dimensions[1]) / 2;
  ...
  draw() {
    this.context.save();
    // this.context.clearRect(this.position[0], this.position[1],this.dimensions[0], this.dimensions[1])
      ...
      if (!game.map.objectsArr[i].processMovement(game.currentFrameTime)) {
        ...
        if (
          ...
          if (game.map.objectsArr[i].canMoveRight()) {
            game.map.objectsArr[i].direction =
              game.map.objectsArr[i].directions.right;
          ...
          if (game.map.objectsArr[i].canMoveDown()) {
            game.map.objectsArr[i].direction =
              game.map.objectsArr[i].directions.down;
          ...
          if (game.map.objectsArr[i].canMoveLeft()) {
            game.map.objectsArr[i].direction =
              game.map.objectsArr[i].directions.left;
